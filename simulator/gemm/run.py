...
parser.add_argument("--inputs", help="optional .npz file containing A and B arrays")
parser.add_argument("--c-out", help="optional .npy file for saving the Cerebras result")
...
if args.inputs:
  ...
  if A.shape != (M, K):
    raise ValueError(f"A has shape {A.shape}, expected {(M, K)}")
  if B.shape != (K, N):
    raise ValueError(f"B has shape {B.shape}, expected {(K, N)}")
else:
